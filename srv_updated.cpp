        ...
        for (int i = 0; i < num_events; ++i) {
            if (events[i].data.fd == server_socket) {
                ...
                if (round_in_progress) {
                    ...
                    player_points[client_socket] = 10; // Każdy gracz zaczyna z 10 punktami
                }

                std::cout << "New client connected: " << client_socket << "\n";
                const char* message = "Are you ready?\n";
                ...
                if (bytes_received > 0) {
                    ...
                    if (!round_in_progress && message == "ready\n") {
                        client_ready[client_socket] = true;
                        std::cout << "Client " << client_socket << " is ready.\n";
                        ...
                        if (message == current_taboo + "\n") {
                            if (std::find(guess_order.begin(), guess_order.end(), client_socket) == guess_order.end()) {
                                guess_order.push_back(client_socket);
                                int points = clients.size() - guess_order.size() + 1;
                                player_points[client_socket] += points;

                                std::string success_message = "Zgadłeś hasło! Otrzymujesz " + std::to_string(points) + " punktów!\n";
                                send(client_socket, success_message.c_str(), success_message.size(), 0);

                                std::string broadcast_message = "Gracz " + std::to_string(client_socket) + " odgadł hasło!\n";
                                for (int client : clients) {
